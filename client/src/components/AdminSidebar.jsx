...
export default function AdminSidebar({ admin, onLogout, isOpen, onClose }) {
  ...
  const menuItems = [
    ...
    {
      path: "/admin/healthcare-profiles",
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-lg mb-1.5 transition-colors text-sm ${
                location === item.path
            ...
            >
              ...
              <span className="truncate inline-block max-w-[180px]">{item.label}</span>
            </button>
          ))}
        </nav>
